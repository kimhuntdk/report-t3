...
function Count_Faculty_Master($faculty,$id){
    $mysqli = connect();
    $sql_count = "SELECT report_t3_graduate.std_id  FROM report_t3_graduate LEFT JOIN report_t3_faculty ON report_t3_graduate.faculty_name=report_t3_faculty.faculty_name_th WHERE  ";
    $sql_count .= " report_t3_graduate.faculty_name='$faculty'  AND (report_t3_graduate.degree_name='ปริญญาโท ระบบในเวลาราชการ' OR report_t3_graduate.degree_name='ปริญญาโท ระบบนอกเวลาราชการ' ) AND report_t3_graduate.round_id=$id  GROUP BY report_t3_graduate.faculty_name ";
    $rs_count = $mysqli->query($sql_count);
    $num_count = $rs_count->num_rows;
    return $num_count;
}
...
function Count_Major_Master_weight($major,$weight,$id){
    $mysqli = connect();
    $sql_count = "SELECT report_t3_graduate.std_id  FROM report_t3_graduate LEFT JOIN report_t3_faculty ON report_t3_graduate.faculty_name=report_t3_faculty.faculty_name_th WHERE  ";
    $sql_count .= " report_t3_graduate.major_name='$major' AND report_t3_graduate.round_id=$id   AND report_t3_graduate.weight='$weight'  AND (report_t3_graduate.degree_name='ปริญญาโท ระบบในเวลาราชการ' OR report_t3_graduate.degree_name='ปริญญาโท ระบบนอกเวลาราชการ' ) ";
    //echo $sql_count;
    $rs_count = $mysqli->query($sql_count);
    $num_count = $rs_count->num_rows;
    return $num_count;
...
$content = '';
$content .= '<h3 style="text-align: center; vertical-align: middle;">สรุปจำนวนนิสิตและบทความวิทยานิพนธ์ปริญญาโทที่ตีพิมพ์เผยแพร่ในระดับชาติและระดับนานาชาติ ที่สำเร็จการศึกษา ประจำเดือน  '.$mount.'</h3>';
...
//คณะ // สาขา
$sql_data = "SELECT report_t3_graduate.faculty_name FROM report_t3_graduate LEFT JOIN report_t3_faculty  ON report_t3_graduate.faculty_name=report_t3_faculty.faculty_name_th  WHERE report_t3_faculty.faculty_group='$row_group[group_id]' AND report_t3_graduate.round_id=$id  AND report_t3_graduate.round_id=1 AND (report_t3_graduate.degree_name='ปริญญาโท ระบบในเวลาราชการ' OR report_t3_graduate.degree_name='ปริญญาโท ระบบนอกเวลาราชการ' ) GROUP BY report_t3_graduate.faculty_name";
...
$faculty_name =   $row_data['faculty_name']; 
 ...
 if($num_group>0){
    ...
    $content .='<tr>
     ...
     $sql_fac = "SELECT * FROM report_t3_graduate WHERE faculty_name='$faculty_name' AND round_id=$id  AND (report_t3_graduate.degree_name='ปริญญาโท ระบบในเวลาราชการ' OR report_t3_graduate.degree_name='ปริญญาโท ระบบนอกเวลาราชการ' ) GROUP BY report_t3_graduate.major_name ";
    ...
    foreach ($rs_fac  as  $row_fac) {
        ...
        <td style="border-bottom:1px solid black; border-left:none; border-right:1px solid black; border-top:none; vertical-align:top; text-align: center; vertical-align: middle; width:20px">';
        $sum_in_out=intval(Count_Major_Master_In($row_fac['major_name'],$id)+Count_Major_Master_Out($row_fac['major_name'],$id));
       $content .='<p>'.$sum_in_out.'</p>
        ...
        <p>'.Count_Major_Master_ThesisIS($row_fac['major_name'],'การศึกษาค้นคว้าอิสระ',$id).'</p>
        </td>
        <td style="border-bottom:1px solid black; border-left:none; border-right:1px solid black; border-top:none; vertical-align:top; text-align: center; vertical-align: middle; width:30px">
        <p>'.Count_Major_Master_ThesisIS($row_fac['major_name'],'วิทยานิพนธ์',$id).'</p>
        </td>
        <td style="border-bottom:1px solid black; border-left:none; border-right:1px solid black; border-top:none; vertical-align:top; width:30px">
