...
header('Content-Disposition: attachment; filename="t3-details.doc"');
...
// //ดึงข้อมูลตามระดับ
// $sqlDe = "SELECT * FROM report_t3_degree order by degree_id DESC  ";
...
$content = $css . 
    ...
    $content .= '<tr><td style="text-align: center; vertical-align: middle; height: 150px;"><h4>' . $i . '</h4></td><td style="text-align: center; vertical-align: middle; style="font-size:20pt"">' . $row['std_id'] . '</td><td>' . $row['title'].$row['fname']. ' '.$row['lname']  . '</td><td>' . $row['faculty_name'] . '</td><td>' . $row['major_name'] . '</td><td>';
     if($num_data_tcci==1){ 
      $content .= $row['published_work']; } else {
      foreach($rs_count_tci as $row_count_tci){
        $content.= $row_count_tci['published_work'];
      }
     }
     $content .= '</td><td>' . $row['database'] . '</td><td style="text-align: center; vertical-align: middle;">' . $row['weight'] . '</td><td>' . $row['note'] . '</td></tr>';
     $i++;  
