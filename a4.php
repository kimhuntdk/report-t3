...
$sql_data = "SELECT faculty_name FROM report_t3_graduate WHERE round_id=1 AND (degree_name='ปริญญาโท ระบบในเวลาราชการ' OR degree_name='ปริญญาโท ระบบนอกเวลาราชการ' ) GROUP BY faculty_name";
$rs_data = $mysqli->query($sql_data);
foreach ($rs_data  as  $row_data) {
$faculty_name =   $row_data['faculty_name']; 
// $data_group = [1,2,3];
// foreach($data_group as $valuse){
// $num_cddd=Count_Faculty_GROUP($faculty_name,$valuse);
// $content.= '<tr><td  colspan="12">'.$num_cddd.'</td>';
...
$i=1;
$content.= '<tr><td  colspan="12">'.$faculty_name.'</td>';
...
<p>'.Count_Major_Master_In($row_fac['major_name']).'</p>
</td>
<td style="height: 46px; width: 44.2396px; text-align: center; vertical-align: middle;">
<p>'.Count_Major_Master_Out($row_fac['major_name']).'</p>
</td>
<td style="height: 46px; width: 67.3542px; text-align: center; vertical-align: middle;">
<p>'.Count_Major_Master_In($row_fac['major_name'])+Count_Major_Master_Out($row_fac['major_name']).'</p>
</td>
